...
class Platform
  ...
  def self.get_access_token
    @@redis ||= Redis.new
    expires_at = @@redis.get("platform_access_token_expires_at").to_i
    if expires_at > Time.now.to_i
      @@redis.get("platform_access_token")
    else
  ...
  def self.refresh_access_token
    ...
    if response["errcode"].blank?
      @@redis.set("platform_access_token", response["access_token"])
      @@redis.set("platform_access_token_expires_at", Time.now.to_i + response["expires_in"] - 100)
    end
  ...
  def self.get_jsapi_ticket
    @@redis ||= Redis.new
    expires_at = @@redis.get("platform_jsapi_ticket_expires_at").to_i
    if expires_at > Time.now.to_i
      @@redis.get("platform_jsapi_ticket")
    else
  ...
  def self.refresh_jsapi_ticket
    ...
    if response["errcode"].blank? || response["errcode"].to_s == "0"
      @@redis.set("platform_jsapi_ticket", response["ticket"])
      @@redis.set("platform_jsapi_ticket_expires_at", Time.now.to_i + response["expires_in"] - 100)
    end
    response["ticket"]
  end
  ...
  def self.update_menu
    ...
    response = Platform.post("/cgi-bin/menu/create?access_token=#{self.get_access_token}",
      :body => data.to_json)

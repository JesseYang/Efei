...
class Tablet::HomeworksController < Tablet::ApplicationController
  ...
  def index
    ...
    homeworks = [ ]
    homeworks << pre_test.info_for_tablet("pre_test") if pre_test.present?
    homeworks << post_test.info_for_tablet("post_test") if post_test.present?
    homeworks << exercise.info_for_tablet("exercise") if exercise.present?

...
class Weixin::LessonsController < Weixin::ApplicationController
  ...
  def exercise
    ...
    @pre_test_answer = @pre_test.tablet_answers.where(student_id: @current_user.id).first
    @exercise_answer = @exercise.tablet_answers.where(student_id: @current_user.id).first
    @post_test_answer = @post_test.tablet_answers.where(student_id: @current_user.id).first
  ...
  def time_dist
    @report = @current_user.reports.where(lesson_id: params[:id]).first
